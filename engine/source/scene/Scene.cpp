...
namespace eng
{
	...
	bool Scene::SetParent(GameObject* obj, GameObject* parent)
	{
		...
		if (parent == nullptr)
		{
			...
			//Or 2 the object has been created
			else
			{
				auto it = std::find_if(
					m_objects.begin(),
		...
		//Were trying to add it as a child of another object
		else
		{
			if (currentParent != nullptr)
			{
				...
				{
					bool found = false;
					auto curentElement = parent;

					while (curentElement)
					{
						if (curentElement == obj)
						{
							found = true;
							break;
						}
						curentElement = curentElement->GetParent();
					}
